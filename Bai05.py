...
class App(tk.Tk):
    def __init__(self):
        ...
        self.chk_gioi_tinh = tk.Checkbutton(
            self, variable=self.gioi_tinh, text='Nu')

        self.diem = tk.StringVar()
        self.ent_diem = tk.Entry(
            self, textvariable=self.diem, width=15, justify=tk.RIGHT)

        self.lst_danh_sach = tk.Listbox(self, width=57, font=('Courier New', 12),
                                        activestyle=tk.NONE)

        btn_nhap = tk.Button(self, text='Nhap', width=8,
                             command=self.btn_nhap_click)
